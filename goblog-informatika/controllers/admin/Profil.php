...
class Profil extends CI_Controller {
	...
	public function update_pass()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->personil_model->update_profil($id_personil, "password", $password);
			// 0=login, 1=logout, 2=create, 3=update, 4=delete, 5=resetPass, 6=reply
			$this->log_model->save_log($id_personil, 3, 'Mengupdate password login.');
	...
	public function update_sosmed()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$simpan = $this->sosmed_model->update_sosmed_personil($id_sosmed, $nama_sosmed, $link_sosmed, $ikon_sosmed);
	...
	public function simpan_sosmed()
	{
		...
		$this->form_validation->set_rules('id_personil', 'Pemilik Sosmed', 'trim|required|is_numeric');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->sosmed_model->simpan_sosmed_personil($id_personil, $nama_sosmed, $link_sosmed, $ikon_sosmed);

				// 0=login, 1=logout, 2=create, 3=update, 4=delete, 5=resetPass, 6=reply
			$this->log_model->save_log($id_personil, 1, 'Menambah Akun Sosial Media '.$nama_sosmed.' pribadi.');
	...
	{		
		...
		$id_personil = $this->session->userdata('id');
		$profil = $this->personil_model->get_data_by_id($id_personil);
		foreach ($profil->result() as $row) {
			$data['id_personil'] 	= $row->id;

			$data['aksi_sosmed']	= site_url('admin/profil/simpan_sosmed');
			$data['aksi_upPass']	= site_url('admin/profil/update_pass');
			$data['data_sosmed']	= $this->sosmed_model->get_personil_sosmed($id_personil);
		}

		$this->template->load('admin/template', 'admin/personil/pengaturan_v', $data);
	}

	public function update_foto()
	{
		$id = $this->input->post('id', TRUE);
		$profil = $this->personil_model->get_data_by_id($id)->row();
		$foto_lama = "uploads/personil/".$profil->foto;
		unlink($foto_lama);
		...
		if ($this->upload->do_upload('filefoto')) {
			...
			unlink($foto_lama2);
			$this->personil_model->update_profil($id, "foto", $image);
			// 0=login, 1=logout, 2=create, 3=update, 4=delete, 5=resetPass, 6=reply
	...
	public function compress_tinify($gambar_asli)
	{
		...
		$menuju = './uploads/personil/'.$gambar_asli;
	...
	public function update_usemail()
	{
		...
		$cek = $this->personil_model->cek($field, $value, $id);
		...
		if ($cek->num_rows() > 0) {
			...
			$this->personil_model->update_profil($id, $field, $value);
			foreach ($pilihan as $key => $val) {
				if ($key == $field) {
					$ket = $val;
	...
	{		
		...
		$data_personil = $this->personil_model->get_data_personil_by_username($username)->row();

		if ($this->session->userdata('id') == $data_personil->id) {
			$data['sm_text'] = "Aktivitas Saya";
		}else{
			$data['sm_text'] = $data_personil->full_name;
		}
		$data['log_personil'] = $this->log_model->get_data_by_id($data_personil->id);

		$this->template->load('admin/template', 'admin/personil/aktivitas_v', $data);
	}
	...
	public function index()
	{
		...
		foreach ($profil->result() as $row) {
			...
			$data['personil_info'] 	= $row->personil_info;
			$data['foto'] 			= $row->foto;
			$data['status'] 		= $row->status;
			$data['last_login'] 	= $row->last_login;
			$query2 = $this->db->query("SELECT COUNT(*) as jumlah_artikel FROM tb_artikel WHERE id_author = '$row->id'")->row();
			$data['jml_artikel'] = $query2->jumlah_artikel;
		}

		$data['log_personil'] = $this->log_model->get_data_hari_ini($id_personil);

		$this->template->load('admin/template', 'admin/personil/profil_v', $data);		
	}
