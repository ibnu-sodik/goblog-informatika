				...
				<tbody>
					...
					foreach ($data_berita->result() as $row):
						...
						<tr>
							...
							<td class="text-left"><?= $row->nama_berita; ?></td>
							<td class="text-center"><?= tanggal_indo($row->berita_postdate); ?></td>
							<td class="text-center">
								<?php 
								$file = file_exists(base_url('uploads/berita/'.$row->gambar_berita));
								if (isset($file) && !empty($row->gambar_berita)) {
									$gambar = base_url('uploads/berita/'.$row->gambar_berita);
								}else{
								...
								<div class="ace-thumbnails clearfix">
									<a href="<?= $gambar; ?>" data-rel="colorbox">
										<img width="50" height="50" alt="<?= $row->nama_berita ?>" src="<?= $gambar;; ?>" class="img-circle img-thumbnail" >
									</a>
								</div>
							</td>
							<td class="text-center">
								<a class="green" href="<?= site_url('admin/berita/edit/'.$row->id_berita) ?>" title="Edit">
									<i class="ace-icon fa fa-edit bigger-130"></i>
								</a>

								<a class="red tombol-hapus" href="<?= site_url('admin/berita/hapus/'.$row->id_berita); ?>">
