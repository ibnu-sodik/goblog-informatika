...
class Komentar_model extends CI_Model {
	...
	public function balas($id_komentar, $nama_komentar, $email_komentar, $id_komentar_artikel, $id_author, $website_komentar, $konten_komentar)
	{
		$object = array(
	...
	{
		$query = $this->db->query("SELECT tb_komentar.*, tb_artikel.* FROM tb_komentar JOIN tb_artikel ON id_komentar_artikel = id_artikel WHERE parent_komentar = '0' AND id_author_artikel = '$id_personil'ORDER BY id_komentar DESC LIMIT $offset, $limit ");
		return $query;
	}

	public function get_unview_komentar_by_id_author($id_author)
		{
			$query = $this->db->query("SELECT * FROM tb_komentar WHERE dibaca = 0 AND id_author_artikel = '$id_author'");
			return $query;
		}	
